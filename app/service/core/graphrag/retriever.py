...
class GraphRetriever:
    ...
    def extract_entities_from_question(self, question: str) -> List[str]:
        ...
        entities = set()
        # 中文字符串
        chinese = re.findall(r'[\u4e00-\u9fff]{2,}(?:公司|集团|技术|系统|模型|方法|算法)?', question)
        entities.update(chinese)
        # 英文词
        english = re.findall(r'[A-Z][a-z]+(?: [A-Z][a-z]+)*', question)
        entities.update(english)
    ...
    def hybrid_graph_search(
        ...
        metadata = {
            ...
            "graph_enabled": True,
            "reasoning_paths": unique_paths[:5]  # 添加推理路径
        }
