...
class RedisSessionMemory:
    ...
    def __init__(
        ...
        if redis_client:
            ...
            try:
                ...
                # 删除内存存储降级逻辑，直接抛出异常
                raise RuntimeError(f"Redis连接失败，无法使用会话记忆功能: {e}")
        else:
            self.redis_client = None
            raise ImportError("redis模块未安装且无可用Redis连接，请安装redis模块并确保Redis服务已启动")
    ...
    def get_or_create_session(self, session_id: str = None, user_id: str = "default") -> str:
        ...
        if not self._is_redis_available():
            raise RuntimeError("Redis不可用，无法进行会话操作")
        ...
        if not session_id:
            ...
            if self._session_exists(session_id, None):
                # 会话存在但属于其他用户，创建新ID
                existing_user = self.redis_client.hget(self._get_meta_key(session_id), "user_id")
    ...
    def add_message(self, session_id: str, role: str, content: str,
        ...
        """
        if not self._is_redis_available():
            raise RuntimeError("Redis不可用，无法添加消息")
    ...
    def get_conversation_history(self, session_id: str, user_id: str = "default",
        ...
        """
        if not self._is_redis_available():
            raise RuntimeError("Redis不可用，无法获取对话历史")
    ...
    def get_session_history(self, session_id: str, user_id: str = "default",
        ...
        """
        if not self._is_redis_available():
            raise RuntimeError("Redis不可用，无法获取会话历史")
